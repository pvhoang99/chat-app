...
const Home = () => {
  ...
  const onConnected = () => {
    ...
    stompClient.subscribe('/user/queue/reply', (e) => { console.log(e) });
  }
      ...
      <div className='middle-container'>
        ...
        <div className='middle-container-body'>
          {state.oldMessages?.map((message, key) => {
            return <div className={`chat-content ${message.createdBy === state.me.username ? 'my-chat' : 'friend-chat'}`} key={key}>{message.text}</div>
          })}
        </div>
